...
class CEP78Client {
  ...
  public async fetchContractWASM() {
    if (!this.options.contractWASM) {
      throw new Error('You need to provide a contractWASM');
    }
    return fetchWASM(this.options.contractWASM);
  };
  
  public async fetchTransferCallWASM() {
    if (!this.options.transferCallWASM) {
      throw new Error('You need to provide a transferCallWASM');
    }
  ...
  public async install(
    ...
    const wasmToInstall = wasm || (await this.fetchContractWASM());
  ...
  public async transfer(
    ...
    if (config.useSessionCode) {
      runtimeArgs.insert('nft_contract_hash', this.contractHashKey);
      const wasmToCall = wasm || (await this.fetchTransferCallWASM());
  ...
  public async getOwnerOf(tokenId: string) {
    ...
    return `account-hash-${(result as unknown as CLKeyType).toJSON()}`;
  }
  ...
  public async getMetadataOf(tokenId: string, metadataType?: NFTMetadataKind) {
    ...
    const clMap = result as unknown as CLMapType<CLType, CLType>;
  ...
  public async getBalanceOf(account: CLPublicKey) {
    ...
    return (result as unknown as CLU8Type).toJSON();
  }
